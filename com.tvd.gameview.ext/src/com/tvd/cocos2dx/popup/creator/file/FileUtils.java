...
package com.tvd.cocos2dx.popup.creator.file;
...
public class FileUtils {
	...
	public String readFromFile(String pFilePath) {
		...
		try {
			...
			if(!file.exists()) {
				...
				NotificationCenter.getInstance().putError("File " + pFilePath
	...
	public String readFromFile(IFile file) {
		...
		try {
			if(!file.exists()) {
				...
				NotificationCenter.getInstance().putError("File " 
	...
	public Map<String, Image> fetchDefaultBackgroundImages(String pGroupName,
		...
		try {
			...
			NotificationCenter.getInstance().putError("[Fetch default background image]"
					+ "json format error");
			e.printStackTrace();
		}
		return result;
	...
	public Map<String, Resource> fetchDefaultExitResource(String pGroupName,
		...
		try {
			...
			NotificationCenter.getInstance().putError("[Fetch default exit image]"
					+ "json format error");
			e.printStackTrace();
		}
...
//		System.out.println("content = " + pFileContent
//				+ "\n, group = " + groupRealName
//				+ "\n, result.size = " + result.size());
		NotificationCenter.getInstance().putError(
				"Group '" + pGroupName + "' in default.properties does not exists!");
		return result;
	}
